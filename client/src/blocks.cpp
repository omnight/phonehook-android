...
QString blocks::contactNameFromNumber(QString number) {
    ...
    if(contactNumberCache.contains(number))
        return Native::Instance()->contactName(contactNumberCache[number]);
    ...
    if(sq.next()) {
        QString contact_id = sq.value("contactId").toString();
        contactNumberCache[number] = contact_id;
        return Native::Instance()->contactName(contact_id);
    } else {
        contactNumberCache[number] = "";
        return "";
    }
...
void blocks::addBlockedContact(QString contactId) {

    QSqlQuery sq;
    sq.prepare("INSERT INTO block (type, contact_id) VALUES(1,?)");
    sq.addBindValue(contactId);
    qDebug() << "added block! " << contactId << sq.exec();
...
void blocks::setAllViewed() {
    QSqlQuery clear;
    clear.prepare(R"(
        UPDATE block_history SET viewed = 1;
    )");
