...
void lookup_worker::threadStarted(QMap<QString,QString> parameters, QList<int> botIds) {
    ...
    if(tagWanted == "lookup") {
        QMetaObject::invokeMethod(Native::Instance(),
                                  "sendCommand", Qt::QueuedConnection,
                                  Q_ARG(QString, "setPosition"), Q_ARG(QString, setting::Instance()->get("popup_position", "1")));
    }
    ...
    while(sq.next()) {
        ...
        if(cacheQ.next()) {
            ...
            if(tagWanted == "lookup")
                QMetaObject::invokeMethod(Native::Instance(),
                                          "sendCommand", Qt::QueuedConnection,
                                          Q_ARG(QString, "stateChange"), Q_ARG(QString, "running:" + sq.value("name").toString()));
...
//            dbus_adapter::Instance()->sendQml( displayDataDoc.toJson() );
            ...
            QMetaObject::invokeMethod(Native::Instance(),
                                      "sendCommand", Qt::QueuedConnection,
                                      Q_ARG(QString, "lookupResult"), Q_ARG(QString, displayDataDoc.toJson()));

        } /* else if(tagWanted == "person_search" || tagWanted == "business_search") {
    ...
    if(tagWanted == "lookup") {
        QMetaObject::invokeMethod(Native::Instance(),
                                  "sendCommand", Qt::QueuedConnection,
                                  Q_ARG(QString, "stateChange"), Q_ARG(QString, "finished"));

        // check if popup should timeout
        int popupTimeout = setting::Instance()->get("popup_timeout","0").toInt() * 1000;

        if(popupTimeout != 0)
            QMetaObject::invokeMethod(Native::Instance(),
                                      "sendCommand", Qt::QueuedConnection,
                                      Q_ARG(QString, "hideIn"), Q_ARG(QString, QString::number(popupTimeout)));
    }
