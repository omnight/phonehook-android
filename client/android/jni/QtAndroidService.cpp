...
static void setDisplayMetrics(JNIEnv */*env*/, jclass /*clazz*/,
                            ...
                            jdouble xdpi, jdouble ydpi, jdouble scaledDensity, jdouble what)
{}
...
static jboolean startQtApplication(JNIEnv *env, jobject object, jstring paramsString, jstring environmentString)
{
	...
	for(int i=0; i <= strlen(nativeEnvironmentString); i++) {
		...
		bf[0] = nativeEnvironmentString[i];
		...
		if(bf[0] == '\t' || bf[0] == 0) {
			...
			if(step == 2) {
				::setenv(key, value, 1);
				__android_log_print(ANDROID_LOG_FATAL, "Qt", "setenv : %s :: %s", key, value );
